...
void fill() {
#define TriangleNumber 4
    // Reading from txt------------------------------------------------------------------------------------
    const char* filePath = "../resources/intersecting.txt";
    ...
    else
    {
        for (int i = 0; i < 4 && !feof(mFile); i++)
        {
            for (int j = 0; j < 18 && !feof(mFile); j++)
            {
                ...
                if (j == 0 || j == 3 || j == 6)
                {
                    if (Scanrangemaxx < triangles[i][j])
                        Scanrangemaxx = triangles[i][j];
                    if (Scanrangeminx > triangles[i][j])
                        Scanrangeminx = triangles[i][j];
                }
                if (j == 1 || j == 4 || j == 7)
                {
                    if (Scanrangemaxy < triangles[i][j])
                        Scanrangemaxy = triangles[i][j];
                    if (Scanrangeminy > triangles[i][j])
                        Scanrangeminy = triangles[i][j];
                }
            }
            // Register the edges
        ...
        */
        ...
        for (list<Range>::iterator it1 = ranges.begin(); it1 != ranges.end(); it1++)
        {
            ...
            for (list<Range>::iterator it2 = it1; it2 != ranges.end(); it2++)
            {
                ...
                if ((it1->info.minx.first > it2->info.minx.first && it1->info.minx.first < it2->info.maxx.first) || (it1->info.maxx.first > it2->info.minx.first && it1->info.maxx.first < it2->info.maxx.first) || (it2->info.minx.first > it1->info.minx.first && it2->info.minx.first < it1->info.maxx.first) || (it2->info.maxx.first > it1->info.minx.first && it2->info.maxx.first < it1->info.maxx.first))
                {
                    //两个区间有交集
                    //排序得出三个区间
                ...
                // 如果区间重合
                if ((it1->info.minx.first == it2->info.minx.first && it1->info.maxx.first == it2->info.maxx.first))
                {
                    for (int i = (int)it1->info.minx.first; i < (int)it1->info.maxx.first; i++)
                    {
                        //cout << "it1->cutEdge.getZfromX(i) = " << it1->cutEdge.getZfromX(i) << "\tit2->cutEdge.getZfromX(i) = " << it2->cutEdge.getZfromX(i) << endl;
...
int main(void)
{
    ...
    window = glfwCreateWindow(SCREEN_WIDTH, SCREEN_HEIGHT, "HW2-2", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
