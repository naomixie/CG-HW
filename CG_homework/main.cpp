...
#pragma region Draw Cylinder
    ...
    cylinder_base_line.insert(pair<GLfloat, GLfloat>(0.4f, 0.2f));
...
#pragma region Detect Cut
            ...
            mouseX = -RscrnConversion(mouseX);
            mouseY = LscrnConversion(mouseY);
            if (mouseX > INITIAL_HEAD && mouseX < INITIAL_END && mouseY < INITIAL_RADIUS && mouseY > INITIAL_RADIUS / 2)
            {

                // printf("mouseX = %f\t mouseY = %f\n", mouseX, mouseY);

                printf("INSIDE REASONABLE RANGE\n");
                if (cylinder_base_line.find(mouseX) != cylinder_base_line.end())
                {
                    if (cylinder_base_line.find(mouseX)->second > mouseY)
                    {
                        printf("[CUT]THIS POINT EXISTS AND IT HAS A LARGER RADIUS THAN KNIFE, mouseY = %f \t pointY = %f\n", mouseY, cylinder_base_line.find(mouseX)->second);
                        cylinder_base_line.find(mouseX)->second = mouseY;
                    }
                ...
                else
                {
                    ...
                    for (;
                         iter2 != cylinder_base_line.end(); ++iter2, ++iter)
                    {

                        printf("mouseX = %f\t mouseY = %f\t iter->first = %f\t iter2->first = %f\n", mouseX, mouseY, iter->first, iter2->first);

                        if (iter->first < mouseX && iter2->first > mouseX)
                        {
                            printf("[FOUND MOUSEX = %f RANGE] between x1 = %f\t x2 = %f\n", mouseX, iter->first, iter2->first);
                            aX = iter->first;
                            aY = iter->second;
                            bX = iter2->first;
                            bY = iter2->second;
